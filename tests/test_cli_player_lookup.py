...
from backcaster.cli import run_player
...
def test_run_player_does_not_fallback_to_name_after_fangraphs_id_resolution(
    ...
    monkeypatch.setattr(
        "backcaster.cli.lookup_player_ids",
        lambda _name: pd.DataFrame([{"name_given": "John Smith"}]),
    )
    monkeypatch.setattr(
        "backcaster.cli.resolve_player_lookup",
    ...
    monkeypatch.setattr("backcaster.cli.fetch_season_stats", lambda _year, _kind: league_df)
...
def test_run_player_uses_name_fallback_when_no_fangraphs_id(
    ...
    monkeypatch.setattr(
        "backcaster.cli.project_player",
        lambda *_args, **_kwargs: pd.DataFrame([{"Name": "John Smith"}]),
    )
    monkeypatch.setattr("backcaster.cli._render", lambda *_args, **_kwargs: None)
